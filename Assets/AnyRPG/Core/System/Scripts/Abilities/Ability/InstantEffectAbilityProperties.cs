...
namespace AnyRPG {
    ...
    public class InstantEffectAbilityProperties : BaseAbilityProperties {
        ...
        /*
        public void GetInstantEffectAbilityProperties(InstantEffectAbility effect) {
        ...
        public override bool CanUseOn(Interactable target, IAbilityCaster source, bool performCooldownChecks = true, AbilityEffectContext abilityEffectContext = null, bool playerInitiated = false, bool performRangeChecks = true) {
            ...
            if (!CanSimultaneousCast) {
                if (source.AbilityManager.PerformingAbility) {
                    if (playerInitiated) {
                        source.AbilityManager.ReceiveCombatMessage("Cannot cast " + describableData.DisplayName + ". another cast is in progress");
                    }
                    return false;
                }
            }
