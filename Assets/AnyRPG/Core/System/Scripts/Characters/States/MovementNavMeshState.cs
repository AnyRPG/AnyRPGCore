...
namespace AnyRPG {
    public class MovementNavMeshState : ConfiguredClass, IMovementState {
        ...
        public void Enter(bool isReplay, bool isSilent) {
            //Debug.Log($"{unitController.gameObject.name}.MovementNavMeshState.Enter(isReplay: {isReplay}) frame: {Time.frameCount} tick: {(isSilent ? "N/A" : unitMovementController.CurrentMovementData.SimulatedTick)}");

        }

        public void Exit(bool isReplay, bool isSilent) {
            //Debug.Log($"{unitController.gameObject.name}.MovementNavMeshState.Exit(isReplay: {isReplay}) frame: {Time.frameCount} tick: {(isSilent ? "N/A" : unitMovementController.CurrentMovementData.SimulatedTick)}");
            ...
            if (unitController.UnitAnimator != null) {
                ...
                unitController.UnitAnimator.SetVelocityFromLocal(Vector3.zero, false);
            }
        }
        ...
        public void Update(bool isReplay, double timeInterval) {
            ...
            if (unitMovementController.ReconciledNavMeshAgentVelocity.sqrMagnitude > 0) {
                if (unitController.UnitAnimator != null) {
                    ...
                    unitController.UnitAnimator.SetVelocityFromLocal(unitController.transform.InverseTransformDirection(unitMovementController.ReconciledNavMeshAgentVelocity), true);
                }
            } else {
                //Debug.Log($"{unitController.gameObject.name}.MovementNavMeshState.Update() setting moving false because velocity is zero");
                if (unitController.UnitAnimator != null) {
                    unitController.UnitAnimator.SetMoving(false);
                    unitController.UnitAnimator.SetVelocityFromLocal(Vector3.zero, true);
                }
            }
