...
namespace AnyRPG.EditorTools {
    public abstract class NewCharacterWizardBase : ScriptableWizard {

        protected const string pathToDefaultCharacterUnitPrefab = "/AnyRPG/Core/System/Prefabs/Character/Unit/DefaultCharacterUnit.prefab";
        protected const string pathToUnitSpawnNodeTemplate = "/AnyRPG/Core/Templates/Prefabs/UnitSpawner/UnitSpawnNode.prefab";
        protected const string pathToLeftHandAttachment = "/AnyRPG/Core/Templates/Prefabs/Attachments/LeftHandAttachment.prefab";
        protected const string pathToRightHandAttachment = "/AnyRPG/Core/Templates/Prefabs/Attachments/RightHandAttachment.prefab";
        protected const string pathToLeftArmAttachment = "/AnyRPG/Core/Templates/Prefabs/Attachments/ShieldAttachment.prefab";
        protected const string pathToHipAttachments = "/AnyRPG/Core/Templates/Prefabs/Attachments/HipAttachments.prefab";
        protected const string pathToSpineAttachments = "/AnyRPG/Core/Templates/Prefabs/Attachments/SpineAttachments.prefab";
        protected const string defaultFootstepLoop = "Default Footstep Loop";

        // the height above the highest transform in the unit the nameplate will be set to
        protected const float namePlateHeightAdd = 0.2f;

        // the highest y value of any transform found in the model, for the purposes of determining nameplate height
        protected float highestYTransform = 0f;
        ...
        protected string[] defaultHeadBones = { "Head" };
        protected string[] defaultLeftHandBones = { "LeftHand", "palm_L", "Hand_L", "Hand.L", "L Hand", "LPalm", "Right wrist" };
        protected string[] defaultRightHandBones = { "RightHand", "palm_R", "Hand_R", "Hand.R", "R Hand", "RPalm", "Left wrist" };
        protected string[] defaultLeftArmBones = { "arm_lower_L", "LeftForeArm", "ForeArm_L", "Forearm.L", "L Forearm", "LlowArm", "LeftRadius", "Left elbow", "LowerArm.L" };
        protected string[] defaultHipBones = { "Hips", "LowerSpine", "LowerBack", "spine1", "Pelvis" };
        protected string[] defaultSpineBones = { "chest", "UpperSpine", "Spine4", "Spine3", "Spine2", "Spine1", "Spine", "Torso2", "Torso", "UpperBack" };

        // bone prefixes to ignore when searching rig - these usually indicate IK bones, and not real bones
        protected List<string> ignoreBones = new List<string>() { "IK", "LD", "DEF" };

        // the used file path name for the game
        protected string fileSystemGameName = string.Empty;

        // the used asset path for the Unit Profile
        protected string scriptableObjectAssetPath = string.Empty;
        ...
        protected void CreateCharacter() {
            ...
            if (addWeaponAttachments == true) {
                ...
                characterModel = AddAttachmentsToModel(newModelFolder);
            }

            EditorUtility.DisplayProgressBar("New Character Wizard", "Creating Unit Profile...", 0.3f);
            UnitProfile unitProfileAsset = ScriptableObject.CreateInstance("UnitProfile") as UnitProfile;

            EditorUtility.DisplayProgressBar("New Character Wizard", "Configuring Unit Profile...", 0.4f);
            // setup unit profile properties
            unitProfileAsset.ResourceName = characterName;
            unitProfileAsset.Icon = portraitImage;
            unitProfileAsset.CharacterName = characterName;
            unitProfileAsset.AutomaticPrefabProfile = false;
            unitProfileAsset.UseInlinePrefabProps = true;
            unitProfileAsset.UseInlineVoiceProps = true;
            ...
            if (spineBone != string.Empty) {
                unitProfileAsset.UnitPrefabProps.FloatTransform = spineBone;
            }
            ModifyUnitProfile(unitProfileAsset);

            CreateExtraModelPrefabs(unitProfileAsset, characterModel);
            ...
            AssetDatabase.CreateAsset(unitProfileAsset, scriptableObjectAssetPath);
        ...
        protected virtual void ModifyUnitProfile(UnitProfile unitProfile) {
            // override in derived classes
        }


        protected GameObject AddAttachmentsToModel(string newModelFolder) {
            ...
            if (modelCopyObject != null) {
                ...
                ModifyModelPrefab(modelCopyObject);
        ...
        public virtual void CreateExtraModelPrefabs(UnitProfile unitProfile, GameObject modelCopy) {
            // override in derived classes
        }

        public virtual void ModifyModelPrefab(GameObject modelPrefab) {
            // override in derived classes
        }

        protected bool CheckFilesExist() {
        ...
        protected void CheckHighestPoint() {
            float highestY = 0f;
        ...
        protected void SetDefaultLeftHandBone() {
        ...
        protected void SetDefaultRightHandBone() {
        ...
        protected void SetDefaultLeftArmBone() {
        ...
        protected void SetDefaultHipBone() {
        ...
        protected void SetDefaultSpineBone() {
