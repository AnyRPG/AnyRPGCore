    ...
    public class InteractableRange : MonoBehaviour {
        ...
        private void OnTriggerEnter(Collider collider) {
            ...
            if (_interactable != null) {
                ...
                if ((PlayerManager.MyInstance.MyCharacter.CharacterController as PlayerController).MyInteractables.Count != 0) {
                    // loop through the table and see if the target is already in it.
                    foreach (Interactable interactable in (PlayerManager.MyInstance.MyCharacter.CharacterController as PlayerController).MyInteractables) {
                ...
                if (!isAlreadyInRangeTable) {
                    ...
                    (PlayerManager.MyInstance.MyCharacter.CharacterController as PlayerController).MyInteractables.Add(_interactable);
                }
        ...
        private void OnTriggerExit(Collider collider) {
            ...
            if (_interactable != null) {
                for (int i = 0; i < (PlayerManager.MyInstance.MyCharacter.CharacterController as PlayerController).MyInteractables.Count; i++) {
                    if ((PlayerManager.MyInstance.MyCharacter.CharacterController as PlayerController).MyInteractables[i] == _interactable) {
                        if (_interactable.IsInteracting == true) {
                            _interactable.StopInteract();
                        }
                        (PlayerManager.MyInstance.MyCharacter.CharacterController as PlayerController).MyInteractables.Remove(_interactable);
                        return;
                    }
                }
            }
