...
public class UIManager : MonoBehaviour {
    ...
    public void HandlePlayerUnitSpawn() {
        //Debug.Log("UIManager.HandleCharacterSpawn()");
        ActivatePlayerUI();

        // allow the player ability manager to send us events so we can redraw the ability list when it changes
        SystemEventManager.MyInstance.OnAbilityListChanged += OnAbilityListChanged;
    ...
    public void OnAbilityListChanged(string newAbility) {
        //Debug.Log("UIManager.OnAbilityListChanged(" + (newAbility == null ? "null" : newAbility) + ")");
        ...
        if (actionBarManager != null) {
            if (SystemAbilityManager.MyInstance.GetResource(newAbility).MyAutoAddToBars == true) {
                if (!actionBarManager.AddNewAbility(newAbility)) {
                    //Debug.Log("UIManager.OnAbilityListChanged(): All Ability Bars were full.  unable to add " + newAbility);
                }
            } else {
                //Debug.Log("UIManager.OnAbilityListChanged(): " + newAbility + ".autoaddtobars = false");
            }
        }
    }
