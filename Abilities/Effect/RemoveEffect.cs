            ...
            if (effectTypeNames != null) {
                foreach (string statusEffectType in effectTypeNames) {
                    ...
                    if (tmpStatusEffectType != null) {
                        ...
                        Debug.LogError("SystemAbilityManager.SetupScriptableObjects(): Could not find status effect type: " + statusEffectType + " while inititalizing " + MyName + ".  CHECK INSPECTOR");
                    }
                }
            }
