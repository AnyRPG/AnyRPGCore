...
public class SystemVideoPanelController : WindowContentController {
    ...
    // THINGS THAT YOU WOULD IMPLEMENT IN THE GAME, BUT DON'T HAVE DIRECT SYSTEM QUALITY SETTINGS ASSOCIATED WITH THEM

    /*
    public void CheckAmbientOcclusion() {
        if (PlayerPrefs.GetInt("AmbientOcclusion") == 0) {
            ambientOcclusionButton.DeSelect();
        } else if (PlayerPrefs.GetInt("AmbientOcclusion") == 1) {
            ambientOcclusionButton.Select();
        }
    }
    ...
    public void CheckCameraEffects() {
        if (PlayerPrefs.GetInt("CameraEffects") == 0) {
            cameraEffectsButton.DeSelect();
        } else if (PlayerPrefs.GetInt("CameraEffects") == 1) {
            cameraEffectsButton.Select();
        }
    }

    public void CheckMotionBlur() {
        if (PlayerPrefs.GetInt("MotionBlur") == 0) {
            motionBlurButton.DeSelect();
        } else if (PlayerPrefs.GetInt("MotionBlur") == 1) {
            motionBlurButton.Select();
        }
    }
